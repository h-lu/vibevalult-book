...
```{mermaid}
graph TD
    User["用户/前端"] -->|HTTPS Request| Firewall["Spring Security 守门人"]
    Firewall -->|身份认证| Auth["认证中心 (Auth)"]
    Auth -->|颁发护照| JWT["JWT Token"]
    User -->|携带护照访问| Filter["JWT 过滤器"]
    Filter -->|验签通过| Guard["授权守卫 (@PreAuthorize)"]
    Guard -->|检查权限| Logic["业务逻辑 (Controller)"]
