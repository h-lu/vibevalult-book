...
### 核心概念：Security Filter Chain（安全过滤器链）

Spring Security 的工作原理就像机场安检。每一个请求（旅客）在到达目的地（Controller）之前，必须经过一系列的检查站（Filters）。

```{mermaid}
graph LR
    Request["HTTP请求"] --> Filter1["Filter 1: 身份检查"]
    Filter1 --> Filter2["Filter 2: 权限检查"]
    Filter2 --> Filter3["Filter 3: CSRF检查"]
    Filter3 --> ...
    ... --> Controller["你的Controller"]
    
    style Request fill:#f96,stroke:#333
    style Controller fill:#9f6,stroke:#333
```

如果任何一个 Filter 觉得请求有问题（比如没带身份证），它会直接拒绝请求（抛出 401/403 异常），请求永远不会到达 Controller。
...
```{mermaid}
graph TD
    subgraph Stateful["有状态 (Session)"]
        User1["用户"] -->|登录| Server1["服务器A (存Session)"]
        User1 -->|请求| Server2["服务器B (无Session)"]
        Server2 -.->|❌ 失败| User1
    end
    
    subgraph Stateless["无状态 (JWT)"]
        User2["用户"] -->|登录| ServerA["服务器A"]
        ServerA -->|返回Token| User2
        User2 -->|携带Token请求| ServerB["服务器B"]
        ServerB -->|✅ 成功| User2
    end
    
    style Stateful fill:#fdd,stroke:#333
    style Stateless fill:#dfd,stroke:#333
```
